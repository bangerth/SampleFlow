...
//
// The connections_01 test already checks that this works when using
// named objects c2...c7. This test also checks what happens if we
...
int main ()
{
  ...
  // Now try some variations with parentheses and ensure that the output is the same
  {
    ...
    auto chain = ((((range_producer >> make_filter(2)) >> make_filter(3)) >> make_filter(5)) >> make_filter(7)) >> stream_output;
    range_producer.sample (std::views::iota(1,100));
